            ...
            <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
                    ...
                    <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex min-w-0 items-center gap-4">
                            ...
                            <div className="min-w-0">
                                ...
                                {editingName ? <div className="mt-1 grid grid-cols-[minmax(0,1fr)_44px_44px] items-center gap-2"><input value={editName} onChange={(event) => { setEditName(event.target.value); setProfileFeedback(null); }} onKeyDown={(event) => { if (event.key === 'Enter') void saveName(); if (event.key === 'Escape') setEditingName(false); }} aria-label="Your name" autoComplete="name" autoFocus className="min-h-11 min-w-0 rounded-lg border border-violet-300 bg-white/10 px-3 text-base font-black text-white outline-none placeholder:text-white/50 focus:border-white sm:text-xl" /><button type="button" aria-label="Save name" onClick={() => void saveName()} disabled={saving} className="grid h-11 w-11 place-items-center rounded-lg bg-white text-emerald-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white disabled:opacity-50">{saving ? <Loader2 size={17} className="animate-spin" /> : <Check size={17} />}</button><button type="button" aria-label="Cancel name edit" onClick={() => { setEditingName(false); setProfileFeedback(null); }} disabled={saving} className="grid h-11 w-11 place-items-center rounded-lg text-white/70 hover:bg-white/10 focus-visible:outline-2 focus-visible:outline-white"><X size={17} /></button></div> : <div className="mt-1 flex min-w-0 items-center gap-2"><h1 className="truncate text-2xl font-black tracking-tight text-white sm:text-3xl">{user.name}</h1><button type="button" aria-label="Edit name" onClick={() => { setEditName(user.name); setProfileFeedback(null); setEditingName(true); }} className="grid h-11 w-11 shrink-0 place-items-center rounded-lg text-white/70 transition hover:bg-white/10 hover:text-white focus-visible:outline-2 focus-visible:outline-white"><Edit2 size={15} /></button></div>}
                ...
                <section className="mt-6 grid gap-4 lg:grid-cols-2">
                    <details className={`${panelClass} group p-5 sm:p-7`}>
                        <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3 rounded-xl focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-violet-700"><div className="flex min-w-0 items-center gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-violet-100 text-violet-700"><BookOpen size={18} /></span><div><h2 className="font-black text-stone-950">Document defaults</h2><p className="mt-0.5 text-sm text-stone-500">Optional header details for your next document.</p></div></div><ChevronRight size={18} className="shrink-0 text-stone-400 transition group-open:rotate-90" /></summary>
